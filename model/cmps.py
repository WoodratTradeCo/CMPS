...
from model.mobilenet_v3 import mobilenetv3_large, h_swish, h_sigmoid
...
class CMPS(nn.Module):
    def __init__(self, cnn, embed_dim=512, num_layers=3, hidden=1024, num_classes=345, img_channel=960, num_patches=49,
        ...
        self.encoder = Encoder(dim_model=embed_dim, num_head=8, hidden=self.hidden, dropout=0.3)
        ...
        self.avgpool_i = nn.AdaptiveAvgPool2d(1)
    ...
    def forward(self, stroke, img):
        ...
        stroke_att = self.softmax(self.avgpool_s(multimodal_stroke))
        output_s = stroke_att * multimodal_stroke + stroke_embed
        ...
        img_att = self.softmax(self.avgpool_i(multimodal_img))
        multimodal_img = img_att * multimodal_img
        ...
        output_i = self.img_classifier(output_i)
